...
class NewsletterApiControllerTest {
        ...
        mockMvc
            .perform(
                post("/api/newsletters/contents")
                    .header("access_token", validToken)
    ...
    @Test
    fun `should return 400 when access_token header is missing`() {
        // Given
        val request =
            CreateContentApiRequest(
                title = "Kotlin 최신 기능 소개",
    ...
    fun `should return 401 when token is invalid`() {
        ...
        mockMvc
            .perform(
                post("/api/newsletters/contents")
                    .header("access_token", invalidToken)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(request))
    ...
    fun `should create content with newsletter source id`() {
        ...
        val request =
            CreateContentApiRequest(
                ...
                publishedAt = LocalDate.now(),
                newsletterSourceId = "source-123",
                contentProviderName = "Test Provider"
            )
