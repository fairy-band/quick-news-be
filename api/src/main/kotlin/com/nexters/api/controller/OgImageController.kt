...
class OgImageController(
    ...
    fun previewOgImage(
        ...
        return generateOgImage(title, tag, newsletterName, textColor)
    }

    @PostMapping("/image")
    fun generateOgImagePost(@RequestBody request: OgImageRequest): ResponseEntity<ByteArray> {
        return generateOgImage(request.title, request.tag, request.newsletterName, request.textColor)
    }
}