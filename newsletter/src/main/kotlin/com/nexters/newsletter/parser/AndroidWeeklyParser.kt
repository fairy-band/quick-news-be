...
class AndroidWeeklyParser : MailParser {
    ...
    private fun parseFromHtml(htmlContent: String, issueInfo: IssueInfo): List<MailContent> {
    ...
    private fun extractSection(htmlContent: String, startMarker: String, endMarker: String): String {
    ...
    private fun parseArticlesSection(sectionHtml: String, issueInfo: IssueInfo): List<MailContent> {
        val results = mutableListOf<MailContent>()
        
        // Android Weekly의 실제 아티클 링크 패턴
        // <a href="...f6p7au8ab..." target="_blank" style="...font-weight: bold...">제목 <span class="main-url">...</span></a>
        val articlePattern = """<a[^>]*href="([^"]*f6p7au8ab[^"]*)"[^>]*target="_blank"[^>]*font-weight:\s*bold[^>]*>([^<]*?)<span class="main-url"[^>]*>[^<]*</span>\s*</a>""".toRegex(RegexOption.DOT_MATCHES_ALL)
    ...
    private fun parseLibrariesSection(sectionHtml: String, issueInfo: IssueInfo): List<MailContent> {
        val results = mutableListOf<MailContent>()
